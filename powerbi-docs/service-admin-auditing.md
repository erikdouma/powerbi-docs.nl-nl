...
Auditing is een functie van Power BI Pro en auditinggebeurtenissen zijn alleen beschikbaar voor gebruikers van Power BI Pro.  Gebruikers met Power BI (gratis licentie) worden weergegeven als **Gratis gebruiker**.

Zie [Purchasing Power BI Pro](service-admin-purchasing-power-bi-pro.md) (Power BI Pro kopen) voor meer informatie over hoe u licenties voor Power BI Pro kunt krijgen en toewijzen aan gebruikers in uw organisatie.

Meer informatie over het beperken van gratis gebruikers die zich registreren vindt u in [Persoonlijke gebruikersregistraties in- of uitschakelen in Azure Active Directory](service-admin-service-free-in-your-organization.md#enable-or-disable-individual-user-sign-up-in-azure-active-directory).
...
| --- | --- |
| Id |Unieke id van een auditrecord. |
| RecordType |Het type bewerking dat wordt aangegeven door de record. Zie de tabel AuditLogRecordType voor meer informatie over de verschillende typen records in auditlogboeken. |
| CreationTime |De datum en tijd in UTC (Coordinated Universal Time) waarop de gebruiker de activiteit heeft uitgevoerd. |
| Operation |De naam van de activiteit door de gebruiker of beheerder. |
| OrganizationId |De GUID voor de Office 365-service van uw organisatie waarin de gebeurtenis heeft plaatsgevonden. |
| UserType |Het type gebruiker dat de bewerking heeft uitgevoerd. Zie de tabel Gebruikerstype voor meer informatie over de verschillende typen gebruikers. |
| UserKey |De unieke id voor Passport van de gebruiker die de activiteit heeft uitgevoerd. |
| ResultStatus |Geeft aan of de actie (opgegeven in de eigenschap Operation) al dan niet is gelukt. Mogelijke waarden zijn Succeeded, PartiallySucceded en Failed. |
| ObjectId |Voor activiteit uit SharePoint en OneDrive voor Bedrijven ziet u hier de volledige padnaam van het bestand of de map die is geraadpleegd door de gebruiker. |
| UserId |De UPN (User Principal Name) van de gebruiker die de actie (opgegeven in de eigenschap Operation) heeft uitgevoerd waardoor de record is vastgelegd, bijvoorbeeld my_name@my_domain_name. Houd er rekening mee dat ook records worden vermeld voor activiteiten die zijn uitgevoerd door systeemaccounts (zoals SHAREPOINT\system of NT AUTHORITY\SYSTEM). |
...
De volgende tabel bevat details van gebeurtenissen die kunnen worden weergegeven.

| **Gebeurtenis** | **Beschrijving** | **Aanvullende details** |
| --- | --- | --- |
...
| Rapport bewerken |Deze activiteit wordt geregistreerd wanneer een rapport wordt gewijzigd. |Rapportnaam, Naam van gegevensset |
...
| Power BI-gegevensset geanalyseerd |Deze activiteit wordt geregistreerd wanneer een Power BI-gegevensset wordt geanalyseerd in Excel. | |

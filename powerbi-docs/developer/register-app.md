...
> [!IMPORTANT]
> Voordat u een Power BI-app registreert, hebt u een [Azure Active Directory-tenant en een organisatiegebruiker](create-an-azure-active-directory-tenant.md) nodig. Als u zich nog niet hebt aangemeld voor Power BI met een gebruiker in uw tenant, wordt de registratie van de app niet voltooid.
> 
> 
...
4. Welk type app u selecteert, is afhankelijk van de toepassing die u gebruikt.
   ...
   * Gebruik een **systeemeigen app** voor apps die worden uitgevoerd op clientapparaten. ***U kiest ook voor een **systeemeigen app** als u inhoud insluit voor uw klanten ongeacht wat de toepassing is. Zelfs voor webtoepassingen.***
5. Voer een waarde in voor **Omleidings-URL** en **URL van startpagina**. Elke geldige URL werkt.
   
    **URL van startpagina** is alleen beschikbaar als u kiest voor een **web-app aan de serverzijde** voor het type toepassing.
...
6. Kies de API's waartoe deze toepassing toegang heeft. Zie [Power BI-machtigingen](power-bi-permissions.md) voor meer informatie over Power BI-toegangsmachtigingen.
...
Wanneer u een toepassing registreert, ontvangt u een **Client-id**.  De **client-id** wordt door de toepassing gebruikt om zich te identificeren bij de gebruikers bij wie ze machtigingen aanvragen.
...
5. **Toepassings-id** wordt vermeld als een GUID. Dit is de client-id voor de toepassing.
...
U moet extra machtigingen inschakelen voor uw toepassing naast de informatie die u hebt opgegeven op de pagina voor de app-registratie. U kunt dit doen via de Azure AD-portal of programmatisch.

U wilt zijn aangemeld met de *hoofd*account, die wordt gebruikt voor insluiting, of een globale beheerdersaccount.
...
4. Selecteer binnen **Vereiste machtigingen** de optie **Power BI Service (Power BI)**.
   ...
   > 
   > 
5. Selecteer alle machtigingen onder **Gedelegeerde machtigingen**. U moet deze één voor één selecteren om ze te kunnen opslaan. Selecteer **Opslaan** wanneer u klaar bent.
   
    ![](media/register-app/powerbi-embedded-azuread-app-permissions04.png)
6. Selecteer binnen **Vereiste machtigingen** de optie **Machtigingen verlenen**.
...
1. U moet de bestaande service-principals (gebruikers) ophalen in uw tenant. Zie [servicePrincipal ophalen](https://developer.microsoft.com/en-us/graph/docs/api-reference/beta/api/serviceprincipal_get) voor informatie over hoe u dit kunt doen.
    ...
    U roept de API *servicePrincipal ophalen* zonder {id} op. Alle service-principals worden dan opgehaald in de tenant.
2. Controleer op een service-principal met uw app-client-id als **appId**-eigenschap.
3. Maak een nieuw service-abonnement als dit ontbreekt voor uw app.
    ...
    ```
4. App-machtiging verlenen aan Power BI API
    ...
    ```
5. App-machtiging verlenen aan AAD
   
    De waarde voor **consentType** hangt af van de gebruiker die de aanvraag doet. U kunt **AllPrincipals** of **Principal** opgeven. **AllPrincipals** kan alleen worden gebruikt door een beheerder om machtigingen te verlenen aan alle gebruikers. **Principal** wordt gebruikt om een machtiging te verlenen aan een specifieke gebruiker. 
   
    Het verlenen van de machtiging is vereist voor de *hoofdaccount* om te voorkomen dat om toestemming wordt gevraagd door Azure AD. 
